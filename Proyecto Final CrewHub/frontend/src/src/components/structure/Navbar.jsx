...
const Navbar = () => {
    ...
    return (
        <Fragment>
            <nav className="fixed top-0 left-0 w-full h-16 bg-[#121212] border-b border-[#262626] z-50 flex items-center justify-between px-4 md:px-8 shadow-sm">

                <div className="flex-1 flex justify-start">
                    <Link to="/home" className="text-white font-black text-lg md:text-xl tracking-widest no-underline">CREW HUB</Link>
                </div>

                {/* ZONA CENTRAL: Solo enlaces principales, oculto en móviles */}
                ...
                <div className="flex-1 flex justify-end items-center gap-2 md:gap-4 relative">
                    ...
                    <div ref={notiRef} className="relative">
                        <button
                            onClick={() => {
                                setIsNotiOpen(!isNotiOpen); setIsProfileOpen(false);
                        ...
                        {isNotiOpen && (
                            <div className="absolute top-14 right-0 w-[350px] bg-[#121212] border border-[#333] rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col">
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-white text-[13px] m-0 leading-tight">
                                                                <strong className="mr-1">{notif.sender?.username || 'Alguien'}</strong>
                    ...
                    <div ref={profileRef} className="relative">
                        <button onClick={() => { setIsProfileOpen(!isProfileOpen); setIsNotiOpen(false); }} className={`w-10 h-10 rounded-full overflow-hidden border-2 transition-colors cursor-pointer block p-0 ${isProfileOpen ? 'border-[#0095f6]' : 'border-[#333] hover:border-[#0095f6]'}`}>
                        ...
                        {isProfileOpen && (
                            <div className="absolute top-14 right-0 w-48 bg-[#121212] border border-[#333] rounded-xl shadow-2xl overflow-hidden z-50 py-2">
                                ...
                                <Link to={`/${activeUser?.username}`} onClick={() => setIsProfileOpen(false)} className="block w-full text-left px-4 py-2.5 text-white text-sm font-bold bg-transparent border-none hover:bg-[#1a1a1a] transition cursor-pointer no-underline">
                                    Ver mi perfil
                                </Link>

                                <Link to="/settings" onClick={() => setIsProfileOpen(false)} className="block w-full text-left px-4 py-2.5 text-white text-sm font-bold bg-transparent border-none hover:bg-[#1a1a1a] transition cursor-pointer no-underline">
                                    Configuración
                                </Link>

                                {(activeUser?.is_admin || activeUser?.username === 'liangbw_') && (
                                    <Link to="/admin" onClick={() => setIsProfileOpen(false)} className="block w-full text-left px-4 py-2.5 text-[#ff4d4d] text-sm font-bold bg-transparent border-none hover:bg-[#1a1a1a] transition cursor-pointer no-underline border-y border-[#262626]">
                                        Panel de Moderación
                                    </Link>
                                )}
