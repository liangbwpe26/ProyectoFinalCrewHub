...
const EditProfile = () => {
    ...
    return (
        <Fragment>
                        ...
                        <div className="mb-5 flex justify-between items-start relative z-10">
                            <div>
                                ...
                                <span className="text-xs text-gray-400 mt-1 block leading-relaxed max-w-[200px] md:max-w-none">
                                    {activeUser?.is_business ? 'Modifica la apariencia de tu empresa para tus clientes.' : 'Activa herramientas comerciales gratis.'}
                                </span>
                            </div>
                        ...
                        {activeUser?.is_business ? (
                            <div className="flex flex-col gap-6 relative z-10">
                                ...
                                <div className="flex flex-col gap-2.5">
                                    ...
                                    <input type="text" value={businessSlogan} onChange={(e) => setBusinessSlogan(e.target.value)} placeholder="Ej. 'Innovando tu futuro cada día'" maxLength={60} className="w-full p-4 rounded-xl border border-[#333] bg-[#0a0a0a]/50 shadow-inner text-[#00ba7c] font-bold outline-none focus:border-[#00ba7c] focus:bg-[#111] transition-all text-sm placeholder:text-gray-600 placeholder:font-normal" />
                                </div>
                                ...
                                <div className="text-sm font-bold bg-[#1a1a1a] p-4 rounded-xl border border-[#333] text-gray-300 flex items-center justify-between shadow-inner">
                                    ...
                                    <span className="text-[#00ba7c] bg-[#00ba7c]/10 border border-[#00ba7c]/20 px-3 py-1 rounded-md uppercase text-[10px] tracking-widest">{activeUser.business_category || 'General'}</span>
                                </div>
                            ...
                            <div className="flex flex-col gap-4 relative z-10">
                                <select value={businessCategory} onChange={(e) => setBusinessCategory(e.target.value)} className="w-full p-4 rounded-xl border border-[#333] bg-[#0a0a0a]/50 shadow-inner text-white font-bold outline-none focus:border-[#00ba7c] focus:bg-[#111] transition-all text-sm cursor-pointer">
                                ...
                                <button type="button" onClick={handleUpgradeToBusiness} disabled={isUpgrading} className={`w-full py-4 rounded-xl font-black text-sm tracking-wide transition-all border-none ${isUpgrading ? 'bg-[#262626] text-gray-500 cursor-wait' : 'bg-gradient-to-r from-[#00ba7c] to-[#008f5e] text-white hover:scale-[1.02] cursor-pointer shadow-[0_0_20px_rgba(0,186,124,0.3)]'}`}>
            ...
            <ConfirmModal isOpen={isDowngradeModalOpen} title="¿Cerrar tienda?" message="¿Seguro que quieres cerrar tu perfil de empresa? Perderás tus métricas y pautas publicitarias." confirmText="Desactivar" cancelText="Cancelar" onConfirm={executeDowngrade} onCancel={() => setIsDowngradeModalOpen(false)} />
        </Fragment>
    );
};
